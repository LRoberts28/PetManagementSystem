...
export default styled(Button)(({ theme, ownerState }) => {
  ...
  const { white, text, gradients, dark } = palette;
  const { boxShadow = () => {}, linearGradient = () => {}, pxToRem, rgba } = functions;
  const { borderRadius } = borders;
  const { colored } = boxShadows;

  // Fix for transparent color handling
  const transparentColor = "rgba(0, 0, 0, 0)"; // Use rgba(0, 0, 0, 0) for transparency
  ...
  const outlinedStyles = () => {
    const backgroundValue = color === "white" ? rgba(white.main, 0.1) : transparentColor;
